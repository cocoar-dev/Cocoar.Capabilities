...
public class CocoarConfigurationIntegrationSpike
{
    ...
    /// </summary>
    private static void ProcessConfigurationCapabilities<T>(IComposition<T> configBag, IServiceCollection services)
        where T : notnull
    {
        // Skip registration if explicitly requested
        if (configBag.Has<SkipRegistrationCapability<T>>())
            return;

        // Determine lifetime and register the main configuration type
        if (configBag.Has<SingletonLifetimeCapability<T>>())
        {
            services.AddSingleton(configBag.Subject);
        }
        else if (configBag.Has<ScopedLifetimeCapability<T>>())
        {
            services.AddScoped<T>();
        }
        else if (configBag.Has<TransientLifetimeCapability<T>>())
        {
