...
public class ExampleUsageTests
{
    ...
    {
        // Arrange - This demonstrates how the capabilities system would be used
        ...
        // Act & Assert - Demonstrate how the DI system would process these capabilities

        // 1. Validation runs first (Order = -100)
        string? validationPattern = null;
        bag.Use<DatabaseConfig, ValidationCapability>(validation =>
        {
        ...
        bag.Use<DatabaseConfig, SingletonLifetimeCapability>(_ =>
        {
            registrationType = "Singleton";
            // In real usage: services.AddSingleton<DatabaseConfig>(dbConfig)
        });

        bag.Use<DatabaseConfig, ScopedLifetimeCapability>(_ =>
        {
        ...
        var contractType = bag.Transform<DatabaseConfig, ExposeAsCapability, Type>(cap => cap.ContractType);
        ...
        var healthCheckName = bag.Transform<DatabaseConfig, HealthCheckCapability, string>(cap => cap.Name);
    ...
    public void CrossProject_Extensibility_DifferentAssembliesAddCapabilities()
    {
        ...
        Assert.True(bag.Contains<SingletonLifetimeCapability>());
        Assert.True(bag.Contains<ExposeAsCapability>());
        Assert.True(bag.Contains<HealthCheckCapability>());
        Assert.True(bag.Contains<ValidationCapability>());
    ...
    public void FluentAPI_BuildsComplexConfigurations_WithMultipleCapabilities()
    {
        ...
        // Verify we can retrieve multiple ExposeAs capabilities
        ...
        // Verify the configuration is complete and usable
    ...
    public void RealWorld_CocoarConfiguration_IntegrationExample()
    {
        ...
        dbBag.Use<DatabaseConfig, SingletonLifetimeCapability>(_ => 
            dbServices.Add("DatabaseConfig as Singleton"));
            
        dbBag.Use<DatabaseConfig, ExposeAsCapability>(expose => 
            dbServices.Add($"Exposed as {expose.ContractType.Name}"));
            
        dbBag.Use<DatabaseConfig, HealthCheckCapability>(health => 
            dbServices.Add($"Health check: {health.Name}"));

        // Cache configuration processing  
        cacheBag.Use<TestSubject, TestCapability>(test => 
            cacheServices.Add($"Cache: {test.Value}"));
